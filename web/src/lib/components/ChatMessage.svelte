...
<div class="flex w-full gap-3 {isUser ? 'flex-row-reverse' : 'flex-row'} items-start">
	{#if isSystem}
		...
		<div class="flex w-fit max-w-[95%] items-start gap-2.5">
			<div
				class="mt-0.5 flex size-6 shrink-0 items-center justify-center rounded-md bg-muted text-muted-foreground/70"
			>
				<Terminal size={13} />
			</div>
			<div
				class="min-w-0 flex-1 rounded-lg bg-muted/40 px-3.5 py-2.5 text-sm text-muted-foreground
				[&_code]:rounded [&_code]:bg-muted [&_code]:px-1
				[&_code]:py-0.5 [&_code]:font-mono [&_code]:text-xs [&_code]:text-foreground [&_li]:my-0.5 [&_p]:my-1
				[&_p:first-child]:mt-0 [&_p:last-child]:mb-0 [&_table]:w-full
				[&_table]:text-xs [&_td]:py-0.5 [&_th]:py-1 [&_th]:text-left [&_th]:font-medium [&_ul]:my-1 [&_ul]:pl-4"
			>
	...
	{:else}
		...
		<!-- Content column -->
		<div class="flex max-w-[95%] min-w-0 flex-col gap-1 {isUser ? 'items-end' : 'items-start'}">
			<!-- Intermediary actions (assistant only) -->
			{#if isAssistant && hasActions}
				<div class="w-fit max-w-full">
					<button
						type="button"
						class="mb-1 flex items-center gap-1.5 text-xs text-muted-foreground transition-colors hover:text-foreground"
