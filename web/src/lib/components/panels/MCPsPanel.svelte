<script lang="ts">
	...
	import { Terminal, Globe, Server, ChevronLeft, Trash2, Plus, X, Tag, ChevronRight, Type } from '@lucide/svelte';
...
{#if isNew || selectedMCP}
	...
	<div class="max-w-2xl mx-auto py-8 px-6 flex flex-col gap-8">
		...
		<div>
			<button
				onclick={() => { selectedMCP = null; isNew = false; }}
			...
			{#if isNew}
				<div class="flex flex-col gap-1.5">
					...
					<div class="relative">
						<Tag class={iconClass} />
						<input id="mcp-name" type="text" bind:value={newName} placeholder="my-mcp" class={inputClass} />
					</div>
				</div>
			{:else}
		...
		<div class="flex flex-col gap-3">
			...
			<div class="grid grid-cols-2 gap-3">
				<button
					...
					class="rounded-xl border-2 p-4 text-left transition-colors {mcpType === 'local' ? 'border-primary bg-primary/5' : 'border-border hover:border-muted-foreground/40'}"
				>
					<Terminal class="h-5 w-5 mb-2 text-primary" />
					<p class="text-sm font-semibold">Local</p>
					<p class="text-xs text-muted-foreground">Run a local process</p>
				</button>
				<button
					onclick={() => mcpType = 'remote'}
					class="rounded-xl border-2 p-4 text-left transition-colors {mcpType === 'remote' ? 'border-primary bg-primary/5' : 'border-border hover:border-muted-foreground/40'}"
				>
					<Globe class="h-5 w-5 mb-2 text-primary" />
			...
			<div class="rounded-xl border bg-card p-6 flex flex-col gap-5">
				<div class="flex items-start gap-3 mb-2">
					...
					<div>
						...
						<p class="text-xs text-muted-foreground mt-0.5">Configure the local process to run</p>
					</div>
				</div>
							...
							<div class="flex flex-col gap-2">
								{#each mcpArgs as arg, i}
									<div class="flex gap-2">
										...
										<button
											onclick={() => removeArg(i)}
											class="flex h-9 w-9 items-center justify-center rounded-md border text-muted-foreground hover:text-destructive hover:border-destructive/50 transition-colors"
										>
											<X class="h-3.5 w-3.5" />
										</button>
									</div>
									...
									<div class="flex gap-2">
										<div class="relative w-1/3">
											...
											<input
												type="text"
												value={envItem.key}
												oninput={(e) => updateEnvKey(i, (e.currentTarget as HTMLInputElement).value)}
												placeholder="KEY"
												class="h-9 rounded-md border bg-background pl-9 pr-3 text-sm font-mono outline-none focus:ring-1 focus:ring-ring w-full"
											/>
										</div>
										<div class="relative flex-1">
											...
											<input
												type="text"
												value={envItem.value}
												oninput={(e) => updateEnvValue(i, (e.currentTarget as HTMLInputElement).value)}
												placeholder="value"
												class="h-9 rounded-md border bg-background pl-9 pr-3 text-sm outline-none focus:ring-1 focus:ring-ring w-full"
											/>
										</div>
			...
			<div class="rounded-xl border bg-card p-6 flex flex-col gap-5">
				<div class="flex items-start gap-3 mb-2">
					...
					<div>
						...
						<p class="text-xs text-muted-foreground mt-0.5">URL of the remote MCP server</p>
					</div>
				</div>
				<div class="flex flex-col gap-1.5">
	...
	<div class="max-w-2xl mx-auto py-8 px-6 flex flex-col gap-6">
		<div class="flex items-center justify-between">
			<div>
				...
				<p class="text-sm text-muted-foreground mt-0.5">Manage your Model Context Protocol servers</p>
			</div>
			<button
				onclick={startNew}
		...
		<div class="flex flex-col gap-2">
			{#each daemon.mcps as mcp (mcp.name)}
				...
				<div class="flex items-center gap-3 rounded-xl border bg-card px-4 py-3 hover:bg-accent/30 transition-colors">
					...
					<div class="flex-1 min-w-0">
						<p class="text-sm font-semibold truncate">{mcp.name}</p>
						<p class="text-xs text-muted-foreground mt-0.5 truncate">
							{type}{cmd ? ` · ${cmd}` : ''}
						</p>
					</div>
					<span class="rounded-full px-2.5 py-0.5 text-xs font-medium bg-muted text-muted-foreground shrink-0">
						{type}
					</span>
					<button
			...
			{#if daemon.mcps.length === 0}
				<p class="text-sm text-muted-foreground py-8 text-center">No MCPs configured</p>
			{/if}
		</div>
	</div>
{/if}
