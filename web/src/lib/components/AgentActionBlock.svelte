...
<div class="rounded-md border border-blue-200/40 bg-blue-500/5 text-xs">
	...
	<!-- Expanded content -->
	{#if expanded}
		<div class="space-y-1 border-t border-blue-200/30 px-3 py-2">
			<div class="flex items-center gap-2">
				<span class="w-20 text-muted-foreground/70">Agent</span>
				<span class="font-medium text-foreground">{action.name}</span>
			</div>
			<div class="flex items-center gap-2">
				<span class="w-20 text-muted-foreground/70">Action</span>
				<span class="text-foreground">{action.action}</span>
			</div>
			<div class="flex items-center gap-2">
				<span class="w-20 text-muted-foreground/70">Started</span>
				<span class="font-mono text-foreground">{formatTime(action.started_at)}</span>
			</div>
			{#if action.finished_at}
				<div class="flex items-center gap-2">
					<span class="w-20 text-muted-foreground/70">Finished</span>
					<span class="font-mono text-foreground">{formatTime(action.finished_at)}</span>
				</div>
			{/if}
		</div>
	{/if}
